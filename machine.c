...
void initialize_machine(int reg_size, int mem_size)
{
    ...
    memory_bank = (int*)malloc(MEMORY_SIZE*sizeof(int));
...
*/
    ...
    int value = get_memory(location);
    return(value);
}

void set_word(int location, int value)
{
    set_memory(location,value);
}
...
int get_register(int reg)
{
    ...
    fprintf(stderr,"Simulator Error: GET: Invalid register number r%d used.\n", reg);
    exit(1);
}
...
void set_register(int reg, int value)
{
    ...
    {
	fprintf(stderr,"Simulator Error: SET: Invalid register number r%d used.\n", reg);
	exit(1);
    }
}
